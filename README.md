...
Ah, I see exactly what happened based on your screenshot!When you copied the text over, your editor (or the way it was copied) completely stripped out all the line breaks (newlines).Markdown relies entirely on physical empty lines to know when to start a new paragraph, create a table, or render a heading. Because it was pasted as one giant, continuous string, GitHub is trying to render it as a single massive paragraph.How to fix it:Delete everything currently in your README.md file.Come back to this message and click the "Copy" icon in the top-right corner of the code block below (do not highlight it manually with your mouse, as that can sometimes mess up the line breaks).Paste it directly into your GitHub file and commit the changes!Here is your pristine, fully formatted code:Markdown<div align="center">
...
│           categories | menu_items | orders | order_items        │
└─────────────────────────────────────────────────────────────────┘
🗃 Database SchemaCode snippeterDiagram
    categories ||--o{ menu_items : "has many"
    orders ||--o{ order_items : "has many"
    menu_items ||--o{ order_items : "referenced by"
...
Bash
git clone [https://github.com/skyp3crack/Rebuild-Restaurant-Smart-Ordering-System.git](https://github.com/skyp3crack/Rebuild-Restaurant-Smart-Ordering-System.git)
cd Rebuild-Restaurant-Smart-Ordering-System/smart-restaurant
Run the automated setup (installs dependencies, generates .env, runs migrations, and builds assets)
...
3. (Optional) Seed the database with a test userBash
php artisan db:seed
...
Navigate to /login and sign in (or register a new account).
Go to Manage Menu Items from the navigation.
Click + Add New Item to create a menu entry.
Fill in the item name, price, category, description, and optionally upload an image.
Use the Edit and Delete actions on the menu table to manage existing items.
...
Access the public menu at /table/{table_number} (e.g., /table/5).
Browse items grouped by category — only available items are shown.
Tap Add to add items to your cart.
Use the + / − buttons in the floating cart to adjust quantities.
Tap Checkout to submit the order to the kitchen.
A success message confirms the order has been placed.

3. Kitchen Dashboard
Log in and navigate to Kitchen Dashboard from the navigation.
View all pending orders displayed as cards in a responsive grid.
Each card shows the table number, order ID, and itemised list with quantities.
Click Mark as Completed to move an order out of the queue.
Use the Refresh Page button to check for new incoming orders.
